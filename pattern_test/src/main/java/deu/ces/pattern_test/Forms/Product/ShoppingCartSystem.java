...
public class ShoppingCartSystem {
    ...
    // 입력된 ID의 사용자에 대한 모든 장바구니 목록 불러옴
    public ArrayList<ShoppingHistory> getSalesHistory(String brandName) {
        ArrayList<ShoppingHistory> getHistory = new ArrayList<>();
        for (ShoppingHistory shoppingHistory : shoppingHistory) {
            try {
                if (brandName.equals(shoppingHistory.getBrandName())) {
                    getHistory.add(shoppingHistory);
                }
            } catch (NullPointerException ex) {
                System.out.println("뭔가 이상한데");
    ...
    public void removeCartItem(String customerID, String productName){
    ...
    private void updatePaymentInfo(String customerID){
